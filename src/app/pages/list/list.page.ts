...
export class ListPage implements OnInit {
  ...
  getItems() {
    ...
    // }
      const identityHeader = {apv_identity: 'test-identity-header-value'};
    this.amplifyService.api().get('apv', '/user/profile', {headers: {'Naber-Tenancy': `${this.user.tenant}`}}).then((res) => {
      if (res && res.length > 0) {
        this.itemList = res[0];
      } else {
        this.itemList = new ToDoList({userId: this.user.id, items: []});
  ...
  console.log('Cannot get user profile: no active user');
  }
}
}
